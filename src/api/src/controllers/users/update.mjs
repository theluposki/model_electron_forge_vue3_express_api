...
export const update = async (req,res) => {
  ...
  const body = req.body;
  try {
    if (validation.isRequired({ id, ...body }, ["id","name", "image", "email", "birthDate", "permission"]))
    return res.status(400).json({ error: validation.isRequired({ id, ...body }, ["id","name", "image", "email", "birthDate", "permission"]) });
